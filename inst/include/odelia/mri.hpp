...
// Returns the number of fast sub-steps taken (for cost accounting).
template <class System>
long mri_macro_step(const System& sys, const MRICoupling& M, const OdeControl& control,
                    double t, double H,
  ...
  auto agg = [&](const std::vector<double>& v) {
    std::vector<double> g(cdim); sys.aggregate(v, g); return g;
  };

  std::vector<std::vector<double>> F(n), Fbar(n);   // slow tendency + its aggregate at each node
  ...
  for (int i = 1; i < n; ++i) {
    ...
    if (dc > 0.0) {
      std::vector<std::vector<double>> a_poly(M.nmat + 1, std::vector<double>(cdim, 0.0));
      a_poly[0] = agg(x);
      ...
      FastLeg<System> leg(sys, u, a_poly, t + M.c[i - 1] * H, dc * H);
      SolverInternal<FastLeg<System>> inner(leg, control);
      inner.advance_adaptive(leg, t + M.c[i] * H);
      u = inner.get_state();
      nfast += (long)inner.get_times().size() - 1;
    }
...
mri_advance(System& sys, const MRICoupling& M, const OdeControl& control,
            const std::vector<double>& macro_times, long* n_fast = nullptr) {
  const size_t nf = sys.fast_size(), ns = sys.slow_size();
  std::vector<double> full(sys.ode_size());
  sys.ode_state(full.begin());
  std::vector<double> u(full.begin(), full.begin() + nf), x(full.begin() + nf, full.end());

  std::vector<std::vector<double>> hist;
  hist.push_back(full);
  if (n_fast) *n_fast = 0;
  for (size_t m = 0; m + 1 < macro_times.size(); ++m) {
    long nf_step = mri_macro_step(sys, M, control, macro_times[m],
                                  macro_times[m + 1] - macro_times[m], x, u);
    if (n_fast) *n_fast += nf_step;
    std::vector<double> s;
    s.reserve(nf + ns);
