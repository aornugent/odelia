...
make_target <- function(true_pars, ctrl) {
  ...
  hist <- runner$history()
  list(times = times,
       observations = as.matrix(hist[, c("x", "y", "z")]),
       obs_index = which(times %in% hist$time))
}

# Observations are passed per call (owned by the least_squares functional), so
# every value_and_gradient goes through this helper rather than a prior set-up call.
vg <- function(solver_xp, tgt, params = NULL, ic = NULL) {
  Solver_value_and_gradient(solver_xp, tgt$times, tgt$observations, tgt$obs_index,
                            ic = ic, params = params)
}
...
test_that("value_and_gradient gradient matches finite differences", {
  ...
  loss <- function(p) vg(d$ptr, tgt, params = p)$value
  vg_guess <- vg(d$ptr, tgt, params = guess)
  ...
  expect_equal(vg_guess$value, loss(guess), tolerance = 1e-12)
  expect_equal(vg_guess$gradient, fd, tolerance = 1e-5)
})
...
test_that("an optimiser over the double handle recovers Lorenz parameters", {
  ...
  vg_memo <- function(p) {
    if (!identical(p, cache_p)) { cache <<- vg(d$ptr, tgt, params = p); cache_p <<- p }
    cache
  }
  res <- optim(guess,
               fn = function(p) vg_memo(p)$value,
               gr = function(p) vg_memo(p)$gradient,
...
test_that("cached active replay is transparent and re-seeds per call (RIF-3)", {
  ...
  new_solver <- function() {
    lz <- LorenzSystem$new(true_pars[1], true_pars[2], true_pars[3])
    lz$set_initial_state(c(1, 1, 1), t0 = 0)
    Lorenz_Solver$new(lz$ptr, ctrl$ptr)
  }
  ...
  d <- new_solver()
  g1  <- vg(d$ptr, tgt, params = p1)  # first call builds the cache
  g2  <- vg(d$ptr, tgt, params = p2)  # reuses cache, re-seeds p2
  g1b <- vg(d$ptr, tgt, params = p1)  # reuses cache, back to p1
  ...
  fresh <- vg(new_solver()$ptr, tgt, params = p1)
